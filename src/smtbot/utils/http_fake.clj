...
(defn matches?
  ...
  [req-spec sent-opts]
  (debug "req-spec " req-spec)
  (debug "sent-opts " sent-opts)
  (every? (fn [[k v]]
            (if (regex? v)
...
(defn predicate
  ...
  (if (fn? req-spec)
    req-spec
    (let [to-match (spy (if (map? req-spec) req-spec {:url req-spec}))]
      #(matches? to-match %))))
...
(defn build-handlers
  ...
  (if (= 0 (rem (count spec) 2))
    (concat
    (spy (map #(apply handler %) (partition 2 spec)))
     [handle-unmatched])
    (throw (IllegalArgumentException.
...
(defn stub-request
  ...
  (let [handlers (build-handlers spec)]
    (fn [orig-fn opts & callback]
      (debug "handler orig-fn "  orig-fn)
      (debug "handler opts "  opts)
      (debug "handler callback "  callback)
      (->> handlers
